...
export function useCreateComment() {
  ...
  return useMutation({
    mutationFn: async ({ postId, content }: { postId: number; content: string }) => {
      // The shared schema for input expects just 'content', but the URL needs 'postId'
      const input = { content };
      const validated = api.comments.create.input.parse(input);
      ...
      if (!res.ok) {
        ...
        if (res.status === 404) {
             const error = api.comments.create.responses[404].parse(await res.json());
             throw new Error(error.message);
        }
